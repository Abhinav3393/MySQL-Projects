...
select c.*, s.purchase_date from sales s
inner join cars c
...
select count(distinct car_id) from sales;

select sp.name, count(s.car_id) as count_of_cars_sold from sales s
join salespersons sp
	on s.salesman_id = sp.salesman_id
group by sp.name
order by  count_of_cars_sold desc;

# 3. What is the total revenue generated by each salesperson?
select sum(cost_$) from cars;

select s.salesman_id, sp.name, sum(c.cost_$) as revenue_generated
from sales s
...
select 
       c.make, c.type, sum(c.cost_$) as total_revenue
from cars c
...
select style, sum(cost_$) as total_revenue_by_SUV
from cars c
join sales s
	on c.car_id = s.car_id
join salespersons sp
